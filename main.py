...
def ProceedVideo(videoname):
    ...
    while k < end:
        ...
        v = Visualizer(frame[:, :, ::-1],
                       metadata=porridge_metadata, scale=1.2)
        ...
        cv2.imwrite(image_file, proc_img)

        # print (begin)
        k += 1

    SaveGistogram(num, AllData)


def SaveGistogram(maximg, fms):
...
def ShowMarked():
    while True:
        ifn = input("Frame number: (path included, enter 0 to leave): ")
        if ifn == "0":
            break
